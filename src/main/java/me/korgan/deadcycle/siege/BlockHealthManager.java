...
public class BlockHealthManager {
    ...
    public void startVisuals() {
        ...
        particleTask = Bukkit.getScheduler().runTaskTimer(plugin, () -> {
            ...
            for (Map.Entry<BlockPos, Integer> e : hp.entrySet()) {
                BlockPos pos = e.getKey();
                if (broken.containsKey(pos)) continue; // если уже сломан — красный приоритет

                int cur = e.getValue();
                int max = getMaxHp(pos.toLocation(w).getBlock().getType());
                if (max <= 0) continue;
                if (cur >= max) continue; // полностью целый
    ...
    public int getMaxHp(Material m) {
        ...
        if (plugin.getConfig().isSet(path)) return direct;
        ...
        if (m == Material.STONE) {
            ...
            if (plugin.getConfig().isSet(cobblePath)) return cobble;
        }
    ...
    /** Нанести урон блоку. Если сломался — превращаем в "искру" (AIR), запоминаем исходный материал. */
    ...
    public int repair(Block b, int amount) {
        if (b == null) return 0;
        if (!plugin.base().isEnabled()) return 0;
        if (!plugin.base().isOnBase(b.getLocation())) return 0;
    ...
    private boolean isWallMaterial(Material m) {
        ...
        for (int lvl = 1; lvl <= maxLevel; lvl++) {
            if (getWallMaterialForLevel(lvl) == m) return true;
        }
    ...
    private boolean isWallLocation(Location loc) {
        ...
        if (plugin.base() == null || plugin.base().getCenter() == null) return false;

        Location c = plugin.base().getCenter();
        if (c.getWorld() == null || loc.getWorld() == null) return false;
        if (!c.getWorld().equals(loc.getWorld())) return false;
        ...
        if (d2 < (double) rMin * rMin || d2 > (double) rMax * rMax) return false;
    ...
    /** Приоритет: сломанные -> поврежденные. Возвращает сколько HP реально восстановлено. */
    ...
    public int getBrokenCountOnBase(Location center, int radius) {
        ...
        for (BlockPos pos : broken.keySet()) {
            if (!pos.isSameWorld(center.getWorld())) continue;
            if (pos.distanceSquared(center) <= r2) c++;
        }
        return c;
    }
    ...
    public int getDamagedCountOnBase() {
        ...
        int c = 0;
        for (Map.Entry<BlockPos, Integer> e : hp.entrySet()) {
            if (broken.containsKey(e.getKey())) continue;
            BlockPos pos = e.getKey();
            World w = Bukkit.getWorld(plugin.base().getWorldName());
            if (w == null) continue;
    ...
    public int getDamagedCountOnBase(Location center, int radius) {
        ...
        for (Map.Entry<BlockPos, Integer> e : hp.entrySet()) {
            BlockPos pos = e.getKey();
            if (!pos.isSameWorld(w)) continue;
            if (pos.distanceSquared(center) > r2) continue;
            if (broken.containsKey(pos)) continue;
    ...
    public int getTotalMissingHpOnBase(Location center, int radius) {
        ...
        for (Map.Entry<BlockPos, Integer> e : hp.entrySet()) {
            ...
            int cur = e.getValue();
            if (cur < max) missing += (max - cur);
        }

        return missing;
    }

    /** Используется апгрейдом стен: очистить красно/оранжевое состояние на этой клетке */
    ...
    // -------- методы для совместимости с RepairGUI (чтоб не падало “cannot resolve”) --------

    public void addActiveRepairer(UUID id) {
        if (id != null) activeRepairers.add(id);
    }

    public void removeActiveRepairer(UUID id) {
        if (id != null) activeRepairers.remove(id);
    }
